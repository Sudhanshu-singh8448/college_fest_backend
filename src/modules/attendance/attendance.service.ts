...
export class AttendanceService {
  ...
  async verifyQr(qrToken: string) {
    ...
    if (!user) throw new NotFoundException('User associated with ticket not found');
  ...
  async checkIn(eventId: string, qrToken: string, actorId: string, hasGlobalPerm: boolean) {
    ...
    if (!['APPROVED', 'COMPLETED', 'CHECKED_IN'].includes(registration.status)) {
      throw new ForbiddenException(`Registration status is ${registration.status}. Cannot check-in.`);
    }
  ...
  async getEventAttendance(eventId: string, actorId: string, hasGlobalPerm: boolean) {
    const event = await this.prisma.event.findUnique({ where: { id: eventId } });
    if (!event) throw new NotFoundException('Event not found');
